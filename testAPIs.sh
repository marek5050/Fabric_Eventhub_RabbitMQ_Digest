...
echo
curl -s -X GET \
  "http://$HOSTPORT/channels/mychannel/chaincodes/mycc?peer=peer0.org1.example.com&fcn=query&args=%5B%22a%22%5D" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
echo
echo
...
echo
curl -s -X GET \
  "http://$HOSTPORT/channels/mychannel/blocks?hash=$HASH&peer=peer0.org1.example.com" \
  -H "authorization: Bearer $ORG1_TOKEN" \
...
echo
curl -s -X GET \
  "http://$HOSTPORT/channels/mychannel?peer=peer0.org1.example.com" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
echo
echo
